...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    div.innerHTML = `<div class="single-product m-2 rounded-3 shadow-lg">
      <div>
    <img class="product-image" src="${image}">
      ...
      <i class="fas fa-star"><span class = 'me-4'>${product.rating.rate}</span></i>
      <i class="fas fa-user-friends"><span >${product.rating.count}</span></i>
      <h3 class="my-2">Price: $ ${product.price}</h3>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-success me-4">add to cart</button>
      <button id="details-btn" class="btn btn-danger" onclick="getDetails(${product.id})" data-bs-toggle="modal" data-bs-target="#exampleModal">Details</button></div>
